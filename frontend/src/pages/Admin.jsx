    ...
    <div className="flex flex-col h-screen bg-background text-foreground font-sans overflow-hidden">
      ...
      <div className="flex items-end justify-between px-8 py-6 border-b shrink-0">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Platform health, user trends, and popular destinations.</p>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        <div className="grid grid-cols-4 gap-4 p-8 pb-4">
          {statCards.map(([k, v, d, Icon, vt]) => (
            <Card key={k} className="rounded-xl border shadow-sm py-0">
          ...
          <Card className="rounded-xl border shadow-sm py-0">
            ...
            <CardContent className="px-4 pb-4">
              <svg viewBox="0 0 600 200" className="w-full h-48">
                {[40, 80, 120, 160].map(y => <line key={y} x1="0" x2="600" y1={y} y2={y} stroke="hsl(var(--border))" />)}
              ...
              {topCities.map(([n, v]) => (
                <div key={n}>
                  ...
                  <div className="h-1.5 rounded-full bg-secondary overflow-hidden">
                    <div className="h-full bg-primary" style={{ width: `${v / 20}%` }} />
                  </div>
                </div>
              ))}
            </CardContent>
            ...
            <CardContent className="p-0">
              <Table>
                ...
                <TableBody>
                  {users.map(r => (
                    <TableRow key={r[0]}>
                      <TableCell className="text-muted-foreground tabular-nums">{r[0]}</TableCell>
                      <TableCell>
